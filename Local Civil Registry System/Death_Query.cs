...
{
    public partial class Death_Query : Form
    {
        public Death_Query()
        {
            InitializeComponent();
        ...
        private async void CheckEntry(string _type)
        {
            ...
            if (_type == "Child" && !string.IsNullOrWhiteSpace(txtChild_1.Text) && !string.IsNullOrWhiteSpace(txtChild_2.Text))
            {
                ...
                var _results = await mc.QueryDeathChild(txtChild_1.Text, txtChild_2.Text);
                TableDesign(_results);
                btn_child_name.Enabled = true;
            }

            else if (_type == "Motherx" && !string.IsNullOrWhiteSpace(txtMother_1.Text) && !string.IsNullOrWhiteSpace(txtMother_2.Text))
            {
            ...
            else if (_type == "Fatherx" && !string.IsNullOrWhiteSpace(txtFather_1.Text) && !string.IsNullOrWhiteSpace(txtFather_2.Text))
            {
